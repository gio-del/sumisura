...
7. **Visual Review (HITL, required).** First run the PDF check over the rendered files — the same page-count check and ATS Report the web app's Render attaches to its own Visual Review, via the same Go code (ADR-0028), plus a check that `data.json` carries a supported `lang`. Whenever this run has a Job Description (pasted, URL or tracked Application — not Default Mode), first write it verbatim to `output/<slug>/job-description.txt` and pass it, so the report covers its terms; add the two Cover Letter flags when a Cover Letter was rendered; always pass `--report-out`, which writes `output/<slug>/ats-report.json`, the same file the app's Render writes:
   ```
   ./plugins/sumisura/skills/tailor-cv/scripts/quality-check.sh pdf --pdf output/<slug>/cv.pdf --data output/<slug>/data.json \
     --job-description output/<slug>/job-description.txt \
     --cover-letter output/<slug>/cover-letter.pdf --cover-letter-data output/<slug>/cover-letter-data.json \
     --report-out output/<slug>/ats-report.json
   ```
   Leave out `--job-description` in Default Mode and the two `--cover-letter` flags when there is no Cover Letter. The output's signals:
   - `Page count:` — anything but 1 is overflow to fix.
   - `ATS-parsability:` — `ok`; `warning`, followed by which expected fields (name, contact details, section headers, employers, project names) were missing from the PDF's extracted text layer or came out in the wrong order, i.e. what an automated screener may not see; or `unavailable` (e.g. `pdftotext` isn't installed), which is a tooling gap and must be reported as "the check couldn't run", never as the PDF being unparsable. Under it, `Contact:` says which of email, phone, LinkedIn and GitHub survived extraction, and `Job Description terms in the text layer: N of M` lists the Job Description's terms that are also tags in the user's own Master Data but aren't on this CV. That line is information for the user, never an instruction to add a term: only Master Data can put one on the CV, through Selection.
   - `Cover Letter ATS-parsability:` — the same verdict for the Cover Letter (name, contact details, opening line).
   - `Language:` — the resolved code, with a warning if `data.json`'s `lang` is missing or unsupported (fix `data.json` and re-render).

   Exit `0` = all clear, `1` = something flagged, `2` = couldn't run (a stderr note, or parsability unavailable with nothing else flagged). None of them stops the pipeline. Term coverage never flags on its own.
